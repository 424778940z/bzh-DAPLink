...
#define USBFS_UIS_H_RES_MASK \
    0x0F // RO, bit mask of current transfer handshake response for USB host mode: 0000=no response, time out from device, others=handshake response
         // PID received
...
//   1 1 1:  dual 64 bytes buffer by bUEP_R_TOG selection for receiving (OUT endpoint) + dual 64 bytes buffer by bUEP_T_TOG selection for transmittal
//   (IN endpoint), total=256bytes
...
//   1 0:  single 64 bytes buffer for endpoint 0 receiving & transmittal (OUT & IN endpoint) + 64 bytes buffer for endpoint 4 receiving (OUT
//   endpoint), total=128bytes 0 1:  single 64 bytes buffer for endpoint 0 receiving & transmittal (OUT & IN endpoint) + 64 bytes buffer for endpoint
//   4 transmittal (IN endpoint), total=128bytes 1 1:  single 64 bytes buffer for endpoint 0 receiving & transmittal (OUT & IN endpoint)
...
#define USBFS_UEP_T_AUTO_TOG \
    0x08 // enable automatic toggle after successful transfer completion on endpoint 1/2/3: 0=manual toggle, 1=automatic toggle
...
#define USBFS_UH_R_RES \
    0x01 // prepared handshake response type for host receiving (IN): 0=ACK (ready), 1=no response, time out to device, for isochronous transactions
...
#define USBFS_UH_T_RES \
    0x01 // expected handshake response type for host transmittal (SETUP/OUT): 0=ACK (ready), 1=no response, time out from device, for isochronous
         // transactions

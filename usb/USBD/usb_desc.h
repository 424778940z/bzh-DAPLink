...
#ifdef DAP_FW_V1
  #define USB_SIZE_CONFIG_TOTAL_ \
      (USB_SIZE_CONFIG_DESC + (USB_SIZE_INTERFACE_DESC + USB_SIZE_HID_DESC + USB_SIZE_ENDPOINT_DESC + USB_SIZE_ENDPOINT_DESC))
#else
  #define USB_SIZE_CONFIG_TOTAL_ (USB_SIZE_CONFIG_DESC + (USB_SIZE_INTERFACE_DESC + USB_SIZE_ENDPOINT_DESC + USB_SIZE_ENDPOINT_DESC))
#endif
#define USB_SIZE_CONFIG_TOTAL     \
    (USB_SIZE_CONFIG_TOTAL_       \
     + (USB_SIZE_IAD_DESC         \
        + USB_SIZE_INTERFACE_DESC \
        + 5                       \
        + 5                       \
        + 4                       \
        + 5                       \
        + USB_SIZE_ENDPOINT_DESC  \
        + USB_SIZE_INTERFACE_DESC \
        + USB_SIZE_ENDPOINT_DESC  \
        + USB_SIZE_ENDPOINT_DESC))

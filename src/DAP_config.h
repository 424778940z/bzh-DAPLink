...
// on CH32V305FBP6, PA8 and PC9 are internally shorted
// on WCH-LinkE, pin 13 is connected to LED
// so both pin actually control the same LED
// as long as they are not both set to output mode it is fine
...
// Control SDI pin for SWD/JTAG/GPIO mode switching
__STATIC_INLINE void sdi_gpio_ctrl(bool sdi_active)
{
    GPIO_PinRemapConfig(GPIO_Remap_SWJ_Disable, sdi_active ? DISABLE : ENABLE);
}
...
__STATIC_FORCEINLINE void PIN_SWDIO_OUT_ENABLE(void)
{
    ...
    SWDIO_PORT->CFGLR = (SWDIO_PORT->CFGLR & ~(0xF << SWDIO_PIN_INDEX * 4)) | (0x3 << SWDIO_PIN_INDEX * 4);
#else
    SWDIO_PORT->CFGHR = (SWDIO_PORT->CFGHR & ~(0xF << (SWDIO_PIN_INDEX - 8) * 4)) | (0x3 << (SWDIO_PIN_INDEX - 8) * 4);
#endif
}
...
#if ( SWDIO_PIN_INDEX < 8 )
    SWDIO_PORT->CFGLR = (SWDIO_PORT->CFGLR & ~(0xF << SWDIO_PIN_INDEX * 4)) | (0x8 << SWDIO_PIN_INDEX * 4);
#else
    SWDIO_PORT->CFGHR = (SWDIO_PORT->CFGHR & ~(0xF << (SWDIO_PIN_INDEX - 8) * 4)) | (0x8 << (SWDIO_PIN_INDEX - 8) * 4);
#endif
}
...
{
    if ( bit & 1 )
        JTAG_TDI_PORT->BSHR = JTAG_TDI_PIN;
    else
        JTAG_TDI_PORT->BCR = JTAG_TDI_PIN;
}
...
{
    if ( bit & 1 )
        nRESET_PORT->BSHR = nRESET_PIN;
    else
        nRESET_PORT->BCR = nRESET_PIN;
}
...
{
    if ( bit & 1 )
        LED_CONNECTED_PORT->BCR = LED_CONNECTED_PIN;
    else
        LED_CONNECTED_PORT->BSHR = LED_CONNECTED_PIN;
}
...
{
    if ( bit & 1 )
        LED_RUNNING_PORT->BCR = LED_RUNNING_PIN;
    else
        LED_RUNNING_PORT->BSHR = LED_RUNNING_PIN;
}
...
{
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE); // Used for SWD/JTAG pin remap

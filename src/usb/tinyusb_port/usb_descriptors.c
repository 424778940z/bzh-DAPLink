...
#endif
#define USBD_VID 0xC251   // Keil Software public example VID
#define USBD_PID 0xF001   // commonly used by community CMSIS-DAP firmwares
                          // (matches the WeAct OEM probe firmware as well)
...
// Endpoint allocation: distinct EP pair per interface so HID, Vendor and CDC
// can coexist. EP numbers are assigned dynamically by the enum below so any
// 2-of-3 combination of {DAP_FW_V1, DAP_FW_V2, DAP_CDC} fits within F411
// OTG_FS's 4-IN-endpoint budget (incl. EP0). _EP0 anchors the count to 0.
enum
{
    _EP0 = 0,
#if defined(DAP_FW_V1)
    _EP_DAP_V1, // HID DAP v1: IN+OUT share one EP number
#endif
#if defined(DAP_FW_V2)
    _EP_DAP_V2, // Vendor DAP v2 / WinUSB: IN+OUT share one EP number
#endif
#if CFG_TUD_CDC
    _EP_CDC_NOTIF, // CDC notification: interrupt IN only
    _EP_CDC_DATA,  // CDC data: bulk IN+OUT share one EP number
#endif
};

#if defined(DAP_FW_V1)
  #define EPNUM_DAP_V1_OUT (_EP_DAP_V1)
  #define EPNUM_DAP_V1_IN  (0x80u | _EP_DAP_V1)
#endif
#if defined(DAP_FW_V2)
  #define EPNUM_DAP_V2_OUT (_EP_DAP_V2)
  #define EPNUM_DAP_V2_IN  (0x80u | _EP_DAP_V2)
#endif
#if CFG_TUD_CDC
  #define EPNUM_CDC_NOTIF (0x80u | _EP_CDC_NOTIF)
  #define EPNUM_CDC_OUT   (_EP_CDC_DATA)
  #define EPNUM_CDC_IN    (0x80u | _EP_CDC_DATA)
#endif
...
static char desc_str_product[] = "BZH STM32F411 CMSIS-DAP";

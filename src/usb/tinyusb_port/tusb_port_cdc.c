...
static void cdc_flush_uart_to_usb(void)
{
    ...
    {
        int avail = uart3_available();
        if ( avail <= 0 )
            break;

        int n = uart3_read((char*)buf, TU_MIN((int)sizeof(buf), avail));
        if ( n <= 0 )
            break;
...
void tud_cdc_line_state_cb(uint8_t itf, bool dtr, bool rts)
{
    ...
    {
        uart3_set_rx_callback(on_uart_rx, 16);
        cdc_flush_uart_to_usb();
    }
}
...
void tud_cdc_line_coding_cb(uint8_t itf, cdc_line_coding_t const* p_line_coding)
{
    ...
    {
        uart3_set_baudrate(p_line_coding->bit_rate);
    }
}
...
void tud_cdc_rx_cb(uint8_t itf)
{
    ...
    while ( tud_cdc_available() )
    {
        ...
        if ( n )
        {
            uart3_write((const char*)buf, (int)n);
        }
    }

...
{
    /* STM32F411 96-bit unique device ID at 0x1FFF7A10 (RM0383 §32.1).
     * On CH32V305 the same ID lived at 0x1FFFF7E8/EC/F0 -- different base. */
    uint32_t IC_UID[3];
    IC_UID[0] = *(uint32_t*)UID_BASE;
    IC_UID[1] = *(uint32_t*)(UID_BASE + 4U);
    IC_UID[2] = *(uint32_t*)(UID_BASE + 8U);
